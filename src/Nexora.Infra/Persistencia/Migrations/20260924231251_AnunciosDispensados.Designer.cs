// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Nexora.Infra.Persistencia;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;

#nullable disable

namespace Nexora.Infra.Persistencia.Migrations
{
    [DbContext(typeof(NexoraDbContext))]
    [Migration("20260924231251_AnunciosDispensados")]
    partial class AnunciosDispensados
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.11")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "abrangencia_feriado_enum", new[] { "nacional", "estadual", "manual" });
            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "direcao_mensagem_enum", new[] { "entrada", "saida" });
            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "evento_webhook_enum", new[] { "lead_criado", "lead_movido", "venda_fechada", "venda_perdida", "mensagem_recebida", "teste" });
            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "fonte_rastreio_enum", new[] { "formulario_site", "anuncio_whatsapp", "importacao" });
            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "origem_lead_enum", new[] { "instagram", "facebook", "whatsapp", "google", "site", "qrcode", "indicacao", "manual", "outro", "meta_ads" });
            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "origem_lembrete_enum", new[] { "automatico", "manual" });
            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "papel_usuario_enum", new[] { "dono", "gestor", "vendedor" });
            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "plataforma_conversao_enum", new[] { "meta" });
            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "resultado_linha_enum", new[] { "importado", "duplicado", "invalido" });
            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "status_conexao_enum", new[] { "nao_criada", "conectando", "conectado", "desconectado", "offline" });
            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "status_conversa_enum", new[] { "aberta", "resolvida" });
            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "status_conversao_enum", new[] { "pendente", "entregue", "falhou", "expirado", "cancelado" });
            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "status_entrega_webhook_enum", new[] { "pendente", "entregue", "falhou" });
            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "status_importacao_enum", new[] { "aguardando_mapeamento", "processando", "concluida", "erro" });
            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "status_lembrete_enum", new[] { "pendente", "concluido", "cancelado" });
            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "status_negociacao_enum", new[] { "aberta", "ganha", "concluida", "perdida", "cancelada" });
            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "status_usuario_enum", new[] { "ativo", "convidado", "inativo" });
            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "tipo_conversao_enum", new[] { "lead", "compra" });
            NpgsqlModelBuilderExtensions.HasPostgresEnum(modelBuilder, "tipo_midia_enum", new[] { "nenhum", "imagem", "documento", "audio", "video" });
            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("Nexora.Core.Entidades.Auditoria", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<string>("Acao")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)")
                        .HasColumnName("acao");

                    b.Property<string>("Alteracoes")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("alteracoes");

                    b.Property<string>("Ator")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("character varying(10)")
                        .HasColumnName("ator");

                    b.Property<long>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.Property<string>("Entidade")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("character varying(30)")
                        .HasColumnName("entidade");

                    b.Property<long>("EntidadeId")
                        .HasColumnType("bigint")
                        .HasColumnName("entidade_id");

                    b.Property<DateTime>("Quando")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("quando");

                    b.Property<long?>("UsuarioId")
                        .HasColumnType("bigint")
                        .HasColumnName("usuario_id");

                    b.HasKey("Id");

                    b.HasIndex("EmpresaId", "Quando")
                        .IsDescending(false, true)
                        .HasDatabaseName("ix_auditoria_empresa");

                    b.HasIndex("EmpresaId", "Entidade", "EntidadeId", "Quando")
                        .IsDescending(false, false, false, true)
                        .HasDatabaseName("ix_auditoria_registro");

                    b.ToTable("auditoria", (string)null);
                });

            modelBuilder.Entity("Nexora.Core.Entidades.CanalCaptacao", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<bool>("Ativo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true)
                        .HasColumnName("ativo");

                    b.Property<DateTime>("AtualizadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("atualizado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<string>("Codigo")
                        .IsRequired()
                        .HasMaxLength(4)
                        .HasColumnType("character varying(4)")
                        .HasColumnName("codigo");

                    b.Property<long>("ConexaoId")
                        .HasColumnType("bigint")
                        .HasColumnName("conexao_id");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<long>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.Property<int>("LeadsRecebidos")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0)
                        .HasColumnName("leads_recebidos");

                    b.Property<string>("MensagemLink")
                        .HasMaxLength(300)
                        .HasColumnType("character varying(300)")
                        .HasColumnName("mensagem_link");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("nome");

                    b.Property<int>("Origem")
                        .HasColumnType("origem_lead_enum")
                        .HasColumnName("origem");

                    b.HasKey("Id");

                    b.HasIndex("EmpresaId", "Codigo")
                        .IsUnique()
                        .HasDatabaseName("uq_canais_empresa_codigo");

                    b.HasIndex("EmpresaId", "Nome")
                        .IsUnique()
                        .HasDatabaseName("uq_canais_empresa_nome");

                    b.ToTable("canais_captacao", (string)null);
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Conexao", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<DateTime>("AtualizadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("atualizado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<DateTime?>("ConectadoEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("conectado_em");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<DateTime?>("DesconectadoEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("desconectado_em");

                    b.Property<long>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.Property<string>("InstanceName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("instance_name");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("nome");

                    b.Property<string>("Numero")
                        .HasColumnType("text")
                        .HasColumnName("numero");

                    b.Property<string>("NumeroAnterior")
                        .HasColumnType("text")
                        .HasColumnName("numero_anterior");

                    b.Property<string>("PerfilFotoUrl")
                        .HasColumnType("text")
                        .HasColumnName("perfil_foto_url");

                    b.Property<string>("PerfilNome")
                        .HasColumnType("text")
                        .HasColumnName("perfil_nome");

                    b.Property<int>("Status")
                        .HasColumnType("status_conexao_enum")
                        .HasColumnName("status");

                    b.Property<DateTime?>("StatusEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("status_em");

                    b.HasKey("Id");

                    b.HasAlternateKey("Id", "EmpresaId")
                        .HasName("uq_conexoes_id_empresa");

                    b.HasIndex("EmpresaId")
                        .HasDatabaseName("ix_conexoes_empresa");

                    b.HasIndex("InstanceName")
                        .IsUnique()
                        .HasDatabaseName("uq_conexoes_instance");

                    b.HasIndex("EmpresaId", "Nome")
                        .IsUnique()
                        .HasDatabaseName("uq_conexoes_empresa_nome");

                    b.ToTable("conexoes", (string)null);
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Contato", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<DateTime?>("AnonimizadoEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("anonimizado_em");

                    b.Property<DateTime>("AtualizadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("atualizado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<string>("Email")
                        .HasColumnType("text")
                        .HasColumnName("email");

                    b.Property<long>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.Property<string>("MetaAdId")
                        .HasColumnType("text")
                        .HasColumnName("meta_ad_id");

                    b.Property<string>("MetaCampaignId")
                        .HasColumnType("text")
                        .HasColumnName("meta_campaign_id");

                    b.Property<string>("MetaFormId")
                        .HasColumnType("text")
                        .HasColumnName("meta_form_id");

                    b.Property<string>("MetaLeadId")
                        .HasColumnType("text")
                        .HasColumnName("meta_lead_id");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("nome");

                    b.Property<string>("Observacoes")
                        .HasColumnType("text")
                        .HasColumnName("observacoes");

                    b.Property<int>("Origem")
                        .HasColumnType("origem_lead_enum")
                        .HasColumnName("origem");

                    b.Property<string>("OrigemDetalhe")
                        .HasColumnType("text")
                        .HasColumnName("origem_detalhe");

                    b.Property<long?>("ResponsavelId")
                        .HasColumnType("bigint")
                        .HasColumnName("responsavel_id");

                    b.Property<string>("Telefone")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("telefone");

                    b.Property<uint>("Versao")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.HasKey("Id");

                    b.HasAlternateKey("Id", "EmpresaId")
                        .HasName("uq_contatos_id_empresa");

                    b.HasIndex("EmpresaId", "CriadoEm")
                        .IsDescending(false, true)
                        .HasDatabaseName("ix_contatos_criado");

                    b.HasIndex("EmpresaId", "MetaLeadId")
                        .IsUnique()
                        .HasDatabaseName("ux_contatos_meta_lead_id")
                        .HasFilter("meta_lead_id IS NOT NULL");

                    b.HasIndex("EmpresaId", "ResponsavelId")
                        .HasDatabaseName("ix_contatos_responsavel")
                        .HasFilter("responsavel_id IS NOT NULL");

                    b.HasIndex("EmpresaId", "Telefone")
                        .IsUnique()
                        .HasDatabaseName("uq_contatos_telefone")
                        .HasFilter("anonimizado_em IS NULL");

                    b.ToTable("contatos", (string)null);
                });

            modelBuilder.Entity("Nexora.Core.Entidades.ContatoEtiqueta", b =>
                {
                    b.Property<long>("ContatoId")
                        .HasColumnType("bigint")
                        .HasColumnName("contato_id");

                    b.Property<long>("EtiquetaId")
                        .HasColumnType("bigint")
                        .HasColumnName("etiqueta_id");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<long?>("CriadoPor")
                        .HasColumnType("bigint")
                        .HasColumnName("criado_por");

                    b.Property<long>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.HasKey("ContatoId", "EtiquetaId");

                    b.HasIndex("EmpresaId", "EtiquetaId")
                        .HasDatabaseName("ix_contatos_etiquetas_etiqueta");

                    b.ToTable("contatos_etiquetas", (string)null);
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Conversa", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<DateTime?>("AguardandoDesde")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("aguardando_desde");

                    b.Property<DateTime?>("AtribuidoEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("atribuido_em");

                    b.Property<DateTime>("AtualizadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("atualizado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<long?>("CanalCicloId")
                        .HasColumnType("bigint")
                        .HasColumnName("canal_ciclo_id");

                    b.Property<long>("ConexaoId")
                        .HasColumnType("bigint")
                        .HasColumnName("conexao_id");

                    b.Property<long>("ContatoId")
                        .HasColumnType("bigint")
                        .HasColumnName("contato_id");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<long>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.Property<int>("NaoLidas")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0)
                        .HasColumnName("nao_lidas");

                    b.Property<DateTime?>("ResolvidoEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("resolvido_em");

                    b.Property<long?>("ResolvidoPor")
                        .HasColumnType("bigint")
                        .HasColumnName("resolvido_por");

                    b.Property<long?>("ResponsavelId")
                        .HasColumnType("bigint")
                        .HasColumnName("responsavel_id");

                    b.Property<int>("Status")
                        .HasColumnType("status_conversa_enum")
                        .HasColumnName("status");

                    b.Property<int?>("UltimaMensagemDirecao")
                        .HasColumnType("direcao_mensagem_enum")
                        .HasColumnName("ultima_mensagem_direcao");

                    b.Property<DateTime>("UltimaMensagemEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ultima_mensagem_em")
                        .HasDefaultValueSql("now()");

                    b.Property<string>("UltimaMensagemPrevia")
                        .HasColumnType("text")
                        .HasColumnName("ultima_mensagem_previa");

                    b.HasKey("Id");

                    b.HasAlternateKey("Id", "EmpresaId")
                        .HasName("uq_conversas_id_empresa");

                    b.HasIndex("ContatoId")
                        .IsUnique()
                        .HasDatabaseName("uq_conversas_contato");

                    b.HasIndex("EmpresaId", "AguardandoDesde")
                        .HasDatabaseName("ix_conversas_aguardando")
                        .HasFilter("aguardando_desde IS NOT NULL");

                    b.HasIndex("EmpresaId", "ResponsavelId", "UltimaMensagemEm")
                        .IsDescending(false, false, true)
                        .HasDatabaseName("ix_conversas_responsavel")
                        .HasFilter("responsavel_id IS NOT NULL");

                    b.HasIndex("EmpresaId", "Status", "UltimaMensagemEm", "Id")
                        .IsDescending(false, false, true, true)
                        .HasDatabaseName("ix_conversas_lista");

                    b.ToTable("conversas", null, t =>
                        {
                            t.HasCheckConstraint("ck_conversas_nao_lidas", "nao_lidas >= 0");
                        });
                });

            modelBuilder.Entity("Nexora.Core.Entidades.CredencialConversao", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<bool>("Ativo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true)
                        .HasColumnName("ativo");

                    b.Property<DateTime>("AtualizadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("atualizado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<string>("CodigoTeste")
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)")
                        .HasColumnName("codigo_teste");

                    b.Property<DateTime?>("ConsentimentoEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("consentimento_em");

                    b.Property<long?>("ConsentimentoPor")
                        .HasColumnType("bigint")
                        .HasColumnName("consentimento_por");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<DateTime?>("DesativadaEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("desativada_em");

                    b.Property<string>("DesativadaMotivo")
                        .HasMaxLength(300)
                        .HasColumnType("character varying(300)")
                        .HasColumnName("desativada_motivo");

                    b.Property<bool>("EmCompra")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true)
                        .HasColumnName("em_compra");

                    b.Property<bool>("EmLead")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true)
                        .HasColumnName("em_lead");

                    b.Property<long>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.Property<string>("Identificador")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("identificador");

                    b.Property<int>("Plataforma")
                        .HasColumnType("plataforma_conversao_enum")
                        .HasColumnName("plataforma");

                    b.Property<string>("Token")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("token");

                    b.HasKey("Id");

                    b.HasIndex("EmpresaId", "Plataforma")
                        .IsUnique()
                        .HasDatabaseName("uq_credenciais_empresa_plataforma");

                    b.ToTable("credenciais_conversao", (string)null);
                });

            modelBuilder.Entity("Nexora.Core.Entidades.EmailEnviado", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<string>("Assunto")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("assunto");

                    b.Property<string>("Destinatario")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("destinatario");

                    b.Property<long?>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.Property<DateTime>("EnviadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("enviado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<string>("Erro")
                        .HasColumnType("text")
                        .HasColumnName("erro");

                    b.Property<bool>("Sucesso")
                        .HasColumnType("boolean")
                        .HasColumnName("sucesso");

                    b.Property<string>("Tipo")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("tipo");

                    b.HasKey("Id");

                    b.HasIndex("Destinatario", "EnviadoEm")
                        .IsDescending(false, true)
                        .HasDatabaseName("ix_emails_destinatario");

                    b.ToTable("emails_enviados", (string)null);
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Empresa", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<DateTime?>("AnunciosDispensadosEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("anuncios_dispensados_em");

                    b.Property<bool>("Ativo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true)
                        .HasColumnName("ativo");

                    b.Property<DateTime>("AtualizadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("atualizado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<bool>("Demonstracao")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("demonstracao");

                    b.Property<short>("DiasParaConcluirVenda")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("smallint")
                        .HasDefaultValue((short)7)
                        .HasColumnName("dias_para_concluir_venda");

                    b.Property<short>("DiasSemRespostaFollowUp")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("smallint")
                        .HasDefaultValue((short)2)
                        .HasColumnName("dias_sem_resposta_followup");

                    b.Property<string>("Documento")
                        .HasColumnType("text")
                        .HasColumnName("documento");

                    b.Property<DateTime?>("EquipeDispensadaEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("equipe_dispensada_em");

                    b.Property<string>("FusoHorario")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasColumnType("text")
                        .HasDefaultValue("America/Sao_Paulo")
                        .HasColumnName("fuso_horario");

                    b.Property<short>("JanelaDiasSemana")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("smallint")
                        .HasDefaultValue((short)126)
                        .HasColumnName("janela_dias_semana");

                    b.Property<short>("JanelaHoraFim")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("smallint")
                        .HasDefaultValue((short)20)
                        .HasColumnName("janela_hora_fim");

                    b.Property<short>("JanelaHoraInicio")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("smallint")
                        .HasDefaultValue((short)8)
                        .HasColumnName("janela_hora_inicio");

                    b.Property<short>("LimiteConexoes")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("smallint")
                        .HasDefaultValue((short)1)
                        .HasColumnName("limite_conexoes");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("nome");

                    b.Property<DateTime?>("OnboardingDispensadoEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("onboarding_dispensado_em");

                    b.Property<DateTime?>("PrimeiraMensagemEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("primeira_mensagem_em");

                    b.Property<short>("SemaforoAmareloMinutos")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("smallint")
                        .HasDefaultValue((short)60)
                        .HasColumnName("semaforo_amarelo_minutos");

                    b.Property<short>("SemaforoVermelhoMinutos")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("smallint")
                        .HasDefaultValue((short)240)
                        .HasColumnName("semaforo_vermelho_minutos");

                    b.Property<string>("Uf")
                        .HasMaxLength(2)
                        .HasColumnType("character(2)")
                        .HasColumnName("uf")
                        .IsFixedLength();

                    b.HasKey("Id");

                    b.ToTable("empresas", null, t =>
                        {
                            t.HasCheckConstraint("ck_empresas_conclusao", "dias_para_concluir_venda BETWEEN 0 AND 90");

                            t.HasCheckConstraint("ck_empresas_dias", "janela_dias_semana BETWEEN 1 AND 127");

                            t.HasCheckConstraint("ck_empresas_hora_faixa", "janela_hora_inicio BETWEEN 0 AND 23 AND janela_hora_fim BETWEEN 1 AND 24");

                            t.HasCheckConstraint("ck_empresas_janela", "janela_hora_inicio < janela_hora_fim");

                            t.HasCheckConstraint("ck_empresas_limite_conexoes", "limite_conexoes BETWEEN 1 AND 20");
                        });
                });

            modelBuilder.Entity("Nexora.Core.Entidades.EntregaWebhook", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<int?>("CodigoResposta")
                        .HasColumnType("integer")
                        .HasColumnName("codigo_resposta");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<bool>("EmMassa")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("em_massa");

                    b.Property<long>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.Property<DateTime?>("EntregueEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("entregue_em");

                    b.Property<string>("Erro")
                        .HasColumnType("text")
                        .HasColumnName("erro");

                    b.Property<int>("Evento")
                        .HasColumnType("evento_webhook_enum")
                        .HasColumnName("evento");

                    b.Property<Guid>("EventoId")
                        .HasColumnType("uuid")
                        .HasColumnName("evento_id");

                    b.Property<string>("Payload")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("payload");

                    b.Property<DateTime?>("ProximaTentativaEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("proxima_tentativa_em");

                    b.Property<int>("Status")
                        .HasColumnType("status_entrega_webhook_enum")
                        .HasColumnName("status");

                    b.Property<short>("Tentativas")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("smallint")
                        .HasDefaultValue((short)0)
                        .HasColumnName("tentativas");

                    b.Property<string>("Url")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("url");

                    b.HasKey("Id");

                    b.HasIndex("CriadoEm")
                        .HasDatabaseName("ix_entregas_criado");

                    b.HasIndex("EmMassa", "ProximaTentativaEm")
                        .HasDatabaseName("ix_entregas_fila")
                        .HasFilter("status = 'pendente'");

                    b.HasIndex("EmpresaId", "Id")
                        .HasDatabaseName("ix_entregas_empresa");

                    b.ToTable("entregas_webhook", (string)null);
                });

            modelBuilder.Entity("Nexora.Core.Entidades.EtapaFunil", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<DateTime>("AtualizadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("atualizado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<string>("Cor")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasColumnType("text")
                        .HasDefaultValue("#2F5D3A")
                        .HasColumnName("cor");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<bool>("EGanho")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("e_ganho");

                    b.Property<long>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("nome");

                    b.Property<short>("Ordem")
                        .HasColumnType("smallint")
                        .HasColumnName("ordem");

                    b.Property<long>("PipelineId")
                        .HasColumnType("bigint")
                        .HasColumnName("pipeline_id");

                    b.HasKey("Id");

                    b.HasAlternateKey("Id", "EmpresaId")
                        .HasName("uq_etapas_id_empresa");

                    b.HasIndex("EmpresaId", "PipelineId")
                        .IsUnique()
                        .HasDatabaseName("uq_etapas_ganho")
                        .HasFilter("e_ganho");

                    b.HasIndex("EmpresaId", "PipelineId", "Ordem")
                        .IsUnique()
                        .HasDatabaseName("uq_etapas_ordem");

                    b.ToTable("etapas_funil", (string)null);
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Etiqueta", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<DateTime>("AtualizadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("atualizado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<string>("Cor")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasColumnType("text")
                        .HasDefaultValue("#2F5D3A")
                        .HasColumnName("cor");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<long?>("CriadoPor")
                        .HasColumnType("bigint")
                        .HasColumnName("criado_por");

                    b.Property<long>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("nome");

                    b.HasKey("Id");

                    b.HasAlternateKey("Id", "EmpresaId")
                        .HasName("uq_etiquetas_id_empresa");

                    b.ToTable("etiquetas", (string)null);
                });

            modelBuilder.Entity("Nexora.Core.Entidades.EventoConversao", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<int?>("CodigoMeta")
                        .HasColumnType("integer")
                        .HasColumnName("codigo_meta");

                    b.Property<int?>("CodigoResposta")
                        .HasColumnType("integer")
                        .HasColumnName("codigo_resposta");

                    b.Property<long>("ContatoId")
                        .HasColumnType("bigint")
                        .HasColumnName("contato_id");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<long>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.Property<DateTime?>("EntregueEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("entregue_em");

                    b.Property<string>("Erro")
                        .HasColumnType("text")
                        .HasColumnName("erro");

                    b.Property<Guid>("EventoId")
                        .HasColumnType("uuid")
                        .HasColumnName("evento_id");

                    b.Property<DateTime>("ExpiraEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("expira_em");

                    b.Property<string>("FbtraceId")
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)")
                        .HasColumnName("fbtrace_id");

                    b.Property<long?>("NegociacaoId")
                        .HasColumnType("bigint")
                        .HasColumnName("negociacao_id");

                    b.Property<DateTime>("OcorridoEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ocorrido_em");

                    b.Property<string>("Payload")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("payload");

                    b.Property<int>("Plataforma")
                        .HasColumnType("plataforma_conversao_enum")
                        .HasColumnName("plataforma");

                    b.Property<DateTime?>("ProximaTentativaEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("proxima_tentativa_em");

                    b.Property<int>("Status")
                        .HasColumnType("status_conversao_enum")
                        .HasColumnName("status");

                    b.Property<short>("Tentativas")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("smallint")
                        .HasDefaultValue((short)0)
                        .HasColumnName("tentativas");

                    b.Property<int>("Tipo")
                        .HasColumnType("tipo_conversao_enum")
                        .HasColumnName("tipo");

                    b.HasKey("Id");

                    b.HasIndex("CriadoEm")
                        .HasDatabaseName("ix_conversoes_criado");

                    b.HasIndex("NegociacaoId")
                        .IsUnique()
                        .HasDatabaseName("uq_conversoes_compra")
                        .HasFilter("tipo = 'compra'");

                    b.HasIndex("ProximaTentativaEm")
                        .HasDatabaseName("ix_conversoes_fila")
                        .HasFilter("status = 'pendente'");

                    b.HasIndex("EmpresaId", "Id")
                        .HasDatabaseName("ix_conversoes_empresa");

                    b.HasIndex(new[] { "ContatoId" }, "ix_conversoes_contato");

                    b.HasIndex(new[] { "ContatoId" }, "uq_conversoes_lead")
                        .IsUnique()
                        .HasFilter("tipo = 'lead'");

                    b.ToTable("eventos_conversao", null, t =>
                        {
                            t.HasCheckConstraint("ck_conversoes_expira", "expira_em > ocorrido_em");

                            t.HasCheckConstraint("ck_conversoes_negociacao", "(tipo = 'compra') = (negociacao_id IS NOT NULL)");
                        });
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Feriado", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<int>("Abrangencia")
                        .HasColumnType("abrangencia_feriado_enum")
                        .HasColumnName("abrangencia");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<DateOnly>("Data")
                        .HasColumnType("date")
                        .HasColumnName("data");

                    b.Property<long?>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("nome");

                    b.Property<string>("Uf")
                        .HasColumnType("text")
                        .HasColumnName("uf");

                    b.HasKey("Id");

                    b.HasIndex("Data")
                        .HasDatabaseName("ix_feriados_data");

                    b.ToTable("feriados", (string)null);
                });

            modelBuilder.Entity("Nexora.Core.Entidades.FeriadoIgnorado", b =>
                {
                    b.Property<long>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.Property<long>("FeriadoId")
                        .HasColumnType("bigint")
                        .HasColumnName("feriado_id");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.HasKey("EmpresaId", "FeriadoId");

                    b.ToTable("feriados_ignorados", (string)null);
                });

            modelBuilder.Entity("Nexora.Core.Entidades.FormularioCaptura", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<bool>("Ativo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true)
                        .HasColumnName("ativo");

                    b.Property<DateTime>("AtualizadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("atualizado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<string>("Chave")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("character varying(64)")
                        .HasColumnName("chave");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<string>("DominioPermitido")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("dominio_permitido");

                    b.Property<long>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.Property<int>("LeadsRecebidos")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0)
                        .HasColumnName("leads_recebidos");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasMaxLength(80)
                        .HasColumnType("character varying(80)")
                        .HasColumnName("nome");

                    b.HasKey("Id");

                    b.HasAlternateKey("Id", "EmpresaId")
                        .HasName("uq_formularios_id_empresa");

                    b.HasIndex("Chave")
                        .IsUnique()
                        .HasDatabaseName("uq_formularios_chave");

                    b.HasIndex("EmpresaId", "Nome")
                        .HasDatabaseName("ix_formularios_empresa");

                    b.ToTable("formularios_captura", (string)null);
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Importacao", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<bool>("AvisarIntegracoes")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("avisar_integracoes");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<int>("Duplicados")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0)
                        .HasColumnName("duplicados");

                    b.Property<long>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.Property<string>("Erro")
                        .HasColumnType("text")
                        .HasColumnName("erro");

                    b.Property<int>("Importados")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0)
                        .HasColumnName("importados");

                    b.Property<int>("Invalidos")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0)
                        .HasColumnName("invalidos");

                    b.Property<string>("Mapeamento")
                        .HasColumnType("jsonb")
                        .HasColumnName("mapeamento");

                    b.Property<string>("NomeArquivo")
                        .IsRequired()
                        .HasMaxLength(260)
                        .HasColumnType("character varying(260)")
                        .HasColumnName("nome_arquivo");

                    b.Property<int>("Origem")
                        .HasColumnType("origem_lead_enum")
                        .HasColumnName("origem");

                    b.Property<long?>("PipelineId")
                        .HasColumnType("bigint")
                        .HasColumnName("pipeline_id");

                    b.Property<DateTime?>("ProcessandoDesde")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("processando_desde");

                    b.Property<long?>("ResponsavelId")
                        .HasColumnType("bigint")
                        .HasColumnName("responsavel_id");

                    b.Property<int>("Status")
                        .HasColumnType("status_importacao_enum")
                        .HasColumnName("status");

                    b.Property<int>("TotalLinhas")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer")
                        .HasDefaultValue(0)
                        .HasColumnName("total_linhas");

                    b.Property<long>("UsuarioId")
                        .HasColumnType("bigint")
                        .HasColumnName("usuario_id");

                    b.HasKey("Id");

                    b.HasIndex("Id")
                        .HasDatabaseName("ix_importacoes_fila")
                        .HasFilter("status = 'processando'");

                    b.HasIndex("EmpresaId", "Id")
                        .IsDescending(false, true)
                        .HasDatabaseName("ix_importacoes_empresa");

                    b.ToTable("importacoes", (string)null);
                });

            modelBuilder.Entity("Nexora.Core.Entidades.ImportacaoLinha", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<long?>("ContatoId")
                        .HasColumnType("bigint")
                        .HasColumnName("contato_id");

                    b.Property<string>("DadosBrutos")
                        .IsRequired()
                        .HasColumnType("jsonb")
                        .HasColumnName("dados_brutos");

                    b.Property<long>("ImportacaoId")
                        .HasColumnType("bigint")
                        .HasColumnName("importacao_id");

                    b.Property<string>("Motivo")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("motivo");

                    b.Property<int>("NumeroLinha")
                        .HasColumnType("integer")
                        .HasColumnName("numero_linha");

                    b.Property<int?>("Resultado")
                        .HasColumnType("resultado_linha_enum")
                        .HasColumnName("resultado");

                    b.HasKey("Id");

                    b.HasIndex("ImportacaoId", "Resultado")
                        .HasDatabaseName("ix_importacao_linhas_resultado");

                    b.ToTable("importacao_linhas", (string)null);
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Lembrete", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<DateTime>("AtualizadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("atualizado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<DateTime?>("ConcluidoEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("concluido_em");

                    b.Property<long?>("ConcluidoPor")
                        .HasColumnType("bigint")
                        .HasColumnName("concluido_por");

                    b.Property<long>("ContatoId")
                        .HasColumnType("bigint")
                        .HasColumnName("contato_id");

                    b.Property<long?>("ConversaId")
                        .HasColumnType("bigint")
                        .HasColumnName("conversa_id");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<long?>("CriadoPor")
                        .HasColumnType("bigint")
                        .HasColumnName("criado_por");

                    b.Property<DateOnly>("DataAlvo")
                        .HasColumnType("date")
                        .HasColumnName("data_alvo");

                    b.Property<long>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.Property<bool>("EnviaMensagem")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("envia_mensagem");

                    b.Property<TimeOnly?>("HoraAlvo")
                        .HasColumnType("time without time zone")
                        .HasColumnName("hora_alvo");

                    b.Property<string>("Observacao")
                        .HasColumnType("text")
                        .HasColumnName("observacao");

                    b.Property<int>("Origem")
                        .HasColumnType("origem_lembrete_enum")
                        .HasColumnName("origem");

                    b.Property<long?>("ResponsavelId")
                        .HasColumnType("bigint")
                        .HasColumnName("responsavel_id");

                    b.Property<int>("Status")
                        .HasColumnType("status_lembrete_enum")
                        .HasColumnName("status");

                    b.Property<string>("TextoMensagem")
                        .HasColumnType("text")
                        .HasColumnName("texto_mensagem");

                    b.Property<string>("Titulo")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("titulo");

                    b.HasKey("Id");

                    b.HasAlternateKey("Id", "EmpresaId")
                        .HasName("uq_lembretes_id_empresa");

                    b.HasIndex("ContatoId", "DataAlvo")
                        .IsUnique()
                        .HasDatabaseName("uq_lembrete_teto_diario")
                        .HasFilter("origem = 'automatico' AND envia_mensagem AND status <> 'cancelado'");

                    b.HasIndex("EmpresaId", "DataAlvo")
                        .HasDatabaseName("ix_lembretes_disparo")
                        .HasFilter("status = 'pendente' AND envia_mensagem");

                    b.HasIndex("EmpresaId", "DataAlvo", "ResponsavelId")
                        .HasDatabaseName("ix_lembretes_dia")
                        .HasFilter("status = 'pendente'");

                    b.ToTable("lembretes", null, t =>
                        {
                            t.HasCheckConstraint("ck_lembretes_texto", "NOT envia_mensagem OR texto_mensagem IS NOT NULL");
                        });
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Mensagem", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<short?>("Ack")
                        .HasColumnType("smallint")
                        .HasColumnName("ack");

                    b.Property<DateTime?>("AckEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ack_em");

                    b.Property<long>("ConexaoId")
                        .HasColumnType("bigint")
                        .HasColumnName("conexao_id");

                    b.Property<long>("ContatoId")
                        .HasColumnType("bigint")
                        .HasColumnName("contato_id");

                    b.Property<long>("ConversaId")
                        .HasColumnType("bigint")
                        .HasColumnName("conversa_id");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<DateOnly?>("DataDisparo")
                        .HasColumnType("date")
                        .HasColumnName("data_disparo");

                    b.Property<int>("Direcao")
                        .HasColumnType("direcao_mensagem_enum")
                        .HasColumnName("direcao");

                    b.Property<long>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.Property<DateTime?>("EnviadaEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("enviada_em");

                    b.Property<long?>("EnviadoPor")
                        .HasColumnType("bigint")
                        .HasColumnName("enviado_por");

                    b.Property<string>("Erro")
                        .HasColumnType("text")
                        .HasColumnName("erro");

                    b.Property<DateTime?>("ExpiradaEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("expirada_em");

                    b.Property<string>("InstanceName")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("instance_name");

                    b.Property<long?>("LembreteId")
                        .HasColumnType("bigint")
                        .HasColumnName("lembrete_id");

                    b.Property<int?>("MidiaBytes")
                        .HasColumnType("integer")
                        .HasColumnName("midia_bytes");

                    b.Property<string>("MidiaChave")
                        .HasColumnType("text")
                        .HasColumnName("midia_chave");

                    b.Property<int?>("MidiaDuracaoSegundos")
                        .HasColumnType("integer")
                        .HasColumnName("midia_duracao_segundos");

                    b.Property<string>("MidiaMime")
                        .HasColumnType("text")
                        .HasColumnName("midia_mime");

                    b.Property<string>("MidiaNome")
                        .HasColumnType("text")
                        .HasColumnName("midia_nome");

                    b.Property<string>("PayloadRaw")
                        .HasColumnType("jsonb")
                        .HasColumnName("payload_raw");

                    b.Property<DateTime?>("RecebidaEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("recebida_em");

                    b.Property<DateTime?>("RecuperadaEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("recuperada_em");

                    b.Property<DateTime>("ReservadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("reservado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<short>("Tentativas")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("smallint")
                        .HasDefaultValue((short)0)
                        .HasColumnName("tentativas");

                    b.Property<string>("Texto")
                        .HasColumnType("text")
                        .HasColumnName("texto");

                    b.Property<int>("TipoMidia")
                        .HasColumnType("tipo_midia_enum")
                        .HasColumnName("tipo_midia");

                    b.Property<string>("WaMessageId")
                        .HasColumnType("text")
                        .HasColumnName("wa_message_id");

                    b.HasKey("Id");

                    b.HasIndex("LembreteId")
                        .IsUnique()
                        .HasDatabaseName("uq_msg_lembrete")
                        .HasFilter("lembrete_id IS NOT NULL");

                    b.HasIndex("EmpresaId", "DataDisparo")
                        .HasDatabaseName("ix_msg_pendentes")
                        .HasFilter("enviada_em IS NULL AND expirada_em IS NULL AND direcao = 'saida'");

                    b.HasIndex("EmpresaId", "RecuperadaEm")
                        .HasDatabaseName("ix_msg_recuperada")
                        .HasFilter("recuperada_em IS NOT NULL");

                    b.HasIndex("InstanceName", "WaMessageId")
                        .IsUnique()
                        .HasDatabaseName("uq_msg_wa_id")
                        .HasFilter("wa_message_id IS NOT NULL AND wa_message_id <> ''");

                    b.HasIndex("EmpresaId", "ConversaId", "Id")
                        .IsDescending(false, false, true)
                        .HasDatabaseName("ix_msg_timeline");

                    b.ToTable("mensagens", null, t =>
                        {
                            t.HasCheckConstraint("ck_msg_ack", "ack IS NULL OR ack BETWEEN 0 AND 4");

                            t.HasCheckConstraint("ck_msg_data_disparo", "direcao = 'entrada' OR data_disparo IS NOT NULL");
                        });
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Negociacao", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<DateTime>("AtualizadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("atualizado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<long?>("CanalCicloId")
                        .HasColumnType("bigint")
                        .HasColumnName("canal_ciclo_id");

                    b.Property<DateTime?>("CanceladaEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("cancelada_em");

                    b.Property<long?>("CanceladaPor")
                        .HasColumnType("bigint")
                        .HasColumnName("cancelada_por");

                    b.Property<DateTime?>("ConcluidaEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("concluida_em");

                    b.Property<long?>("ConcluidaPor")
                        .HasColumnType("bigint")
                        .HasColumnName("concluida_por");

                    b.Property<long>("ContatoId")
                        .HasColumnType("bigint")
                        .HasColumnName("contato_id");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<long>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.Property<long>("EtapaId")
                        .HasColumnType("bigint")
                        .HasColumnName("etapa_id");

                    b.Property<DateTime?>("GanhaEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ganha_em");

                    b.Property<string>("MotivoPerda")
                        .HasColumnType("text")
                        .HasColumnName("motivo_perda");

                    b.Property<string>("Observacao")
                        .HasColumnType("text")
                        .HasColumnName("observacao");

                    b.Property<decimal>("OrdemKanban")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("numeric")
                        .HasDefaultValue(0m)
                        .HasColumnName("ordem_kanban");

                    b.Property<DateTime?>("PerdidaEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("perdida_em");

                    b.Property<long>("PipelineId")
                        .HasColumnType("bigint")
                        .HasColumnName("pipeline_id");

                    b.Property<long?>("ResponsavelId")
                        .HasColumnType("bigint")
                        .HasColumnName("responsavel_id");

                    b.Property<int>("Status")
                        .HasColumnType("status_negociacao_enum")
                        .HasColumnName("status");

                    b.Property<string>("Titulo")
                        .HasColumnType("text")
                        .HasColumnName("titulo");

                    b.Property<decimal?>("Valor")
                        .HasColumnType("numeric(14,2)")
                        .HasColumnName("valor");

                    b.Property<uint>("Versao")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("xid")
                        .HasColumnName("xmin");

                    b.HasKey("Id");

                    b.HasAlternateKey("Id", "EmpresaId")
                        .HasName("uq_negociacoes_id_empresa");

                    b.HasIndex("EmpresaId", "ContatoId")
                        .HasDatabaseName("ix_negociacoes_contato");

                    b.HasIndex("EmpresaId", "GanhaEm")
                        .HasDatabaseName("ix_negociacoes_ganhas")
                        .HasFilter("ganha_em IS NOT NULL AND status <> 'cancelada'");

                    b.HasIndex("EmpresaId", "ContatoId", "PipelineId")
                        .IsUnique()
                        .HasDatabaseName("uq_negociacoes_card_por_funil")
                        .HasFilter("status IN ('aberta', 'ganha')");

                    b.HasIndex("EmpresaId", "EtapaId", "OrdemKanban")
                        .HasDatabaseName("ix_negociacoes_kanban")
                        .HasFilter("status NOT IN ('perdida', 'cancelada')");

                    b.ToTable("negociacoes", null, t =>
                        {
                            t.HasCheckConstraint("ck_negociacoes_terminal", "ganha_em IS NULL OR perdida_em IS NULL");

                            t.HasCheckConstraint("ck_negociacoes_valor", "status IN ('aberta', 'perdida') OR (valor IS NOT NULL AND valor > 0)");
                        });
                });

            modelBuilder.Entity("Nexora.Core.Entidades.NegociacaoEtiqueta", b =>
                {
                    b.Property<long>("NegociacaoId")
                        .HasColumnType("bigint")
                        .HasColumnName("negociacao_id");

                    b.Property<long>("EtiquetaId")
                        .HasColumnType("bigint")
                        .HasColumnName("etiqueta_id");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<long?>("CriadoPor")
                        .HasColumnType("bigint")
                        .HasColumnName("criado_por");

                    b.Property<long>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.HasKey("NegociacaoId", "EtiquetaId");

                    b.HasIndex("EmpresaId", "EtiquetaId")
                        .HasDatabaseName("ix_negociacoes_etiquetas_etiqueta");

                    b.ToTable("negociacoes_etiquetas", (string)null);
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Pipeline", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<DateTime>("AtualizadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("atualizado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<string>("Cor")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasColumnType("text")
                        .HasDefaultValue("#2F5D3A")
                        .HasColumnName("cor");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<long>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("nome");

                    b.Property<short>("Ordem")
                        .HasColumnType("smallint")
                        .HasColumnName("ordem");

                    b.Property<bool>("Padrao")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("padrao");

                    b.HasKey("Id");

                    b.HasAlternateKey("Id", "EmpresaId")
                        .HasName("uq_pipelines_id_empresa");

                    b.HasIndex("EmpresaId")
                        .IsUnique()
                        .HasDatabaseName("uq_pipelines_padrao")
                        .HasFilter("padrao");

                    b.HasIndex("EmpresaId", "Nome")
                        .IsUnique()
                        .HasDatabaseName("uq_pipelines_empresa_nome");

                    b.ToTable("pipelines", (string)null);
                });

            modelBuilder.Entity("Nexora.Core.Entidades.RastreioLead", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<long>("ContatoId")
                        .HasColumnType("bigint")
                        .HasColumnName("contato_id");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<long>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.Property<Guid?>("EventoId")
                        .HasColumnType("uuid")
                        .HasColumnName("evento_id");

                    b.Property<int>("Fonte")
                        .HasColumnType("fonte_rastreio_enum")
                        .HasColumnName("fonte");

                    b.Property<long?>("FormularioId")
                        .HasColumnType("bigint")
                        .HasColumnName("formulario_id");

                    b.Property<string>("Identificadores")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasColumnType("jsonb")
                        .HasColumnName("identificadores")
                        .HasDefaultValueSql("'{}'::jsonb");

                    b.Property<string>("Ip")
                        .HasMaxLength(45)
                        .HasColumnType("character varying(45)")
                        .HasColumnName("ip");

                    b.Property<DateTime>("OcorridoEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ocorrido_em");

                    b.Property<string>("Pagina")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)")
                        .HasColumnName("pagina");

                    b.Property<string>("Referencia")
                        .HasMaxLength(1000)
                        .HasColumnType("character varying(1000)")
                        .HasColumnName("referencia");

                    b.Property<string>("UserAgent")
                        .HasMaxLength(512)
                        .HasColumnType("character varying(512)")
                        .HasColumnName("user_agent");

                    b.Property<string>("UtmCampaign")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("utm_campaign");

                    b.Property<string>("UtmContent")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("utm_content");

                    b.Property<string>("UtmMedium")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("utm_medium");

                    b.Property<string>("UtmSource")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("utm_source");

                    b.Property<string>("UtmTerm")
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)")
                        .HasColumnName("utm_term");

                    b.HasKey("Id");

                    b.HasIndex("ContatoId")
                        .IsUnique()
                        .HasDatabaseName("uq_rastreios_contato");

                    b.HasIndex("EmpresaId", "UtmCampaign")
                        .HasDatabaseName("ix_rastreios_campanha");

                    b.ToTable("rastreios_lead", (string)null);
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Usuario", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<DateTime>("AtualizadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("atualizado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<DateTime?>("BloqueadoAte")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("bloqueado_ate");

                    b.Property<DateTime?>("ConviteExpira")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("convite_expira");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("email");

                    b.Property<long>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.Property<short>("FalhasLogin")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("smallint")
                        .HasDefaultValue((short)0)
                        .HasColumnName("falhas_login");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasColumnType("text")
                        .HasColumnName("nome");

                    b.Property<int>("Papel")
                        .HasColumnType("papel_usuario_enum")
                        .HasColumnName("papel");

                    b.Property<DateTime?>("ResetExpira")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("reset_expira");

                    b.Property<string>("SenhaHash")
                        .HasColumnType("text")
                        .HasColumnName("senha_hash");

                    b.Property<int>("Status")
                        .HasColumnType("status_usuario_enum")
                        .HasColumnName("status");

                    b.Property<string>("TokenConvite")
                        .HasColumnType("text")
                        .HasColumnName("token_convite");

                    b.Property<string>("TokenReset")
                        .HasColumnType("text")
                        .HasColumnName("token_reset");

                    b.Property<DateTime?>("UltimoAcessoEm")
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("ultimo_acesso_em");

                    b.HasKey("Id");

                    b.HasAlternateKey("Id", "EmpresaId")
                        .HasName("uq_usuarios_id_empresa");

                    b.HasIndex("TokenConvite")
                        .IsUnique()
                        .HasDatabaseName("uq_usuarios_token_convite")
                        .HasFilter("token_convite IS NOT NULL");

                    b.HasIndex("TokenReset")
                        .IsUnique()
                        .HasDatabaseName("uq_usuarios_token_reset")
                        .HasFilter("token_reset IS NOT NULL");

                    b.HasIndex("EmpresaId", "Status")
                        .HasDatabaseName("ix_usuarios_empresa");

                    b.ToTable("usuarios", null, t =>
                        {
                            t.HasCheckConstraint("ck_usuarios_senha", "status = 'convidado' OR senha_hash IS NOT NULL");
                        });
                });

            modelBuilder.Entity("Nexora.Core.Entidades.WebhookSaida", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasColumnName("id");

                    NpgsqlPropertyBuilderExtensions.UseIdentityAlwaysColumn(b.Property<long>("Id"));

                    b.Property<bool>("Ativo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true)
                        .HasColumnName("ativo");

                    b.Property<DateTime>("AtualizadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("atualizado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<DateTime>("CriadoEm")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasColumnName("criado_em")
                        .HasDefaultValueSql("now()");

                    b.Property<bool>("EmLeadCriado")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true)
                        .HasColumnName("em_lead_criado");

                    b.Property<bool>("EmLeadMovido")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true)
                        .HasColumnName("em_lead_movido");

                    b.Property<bool>("EmMensagemRecebida")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("em_mensagem_recebida");

                    b.Property<bool>("EmVendaFechada")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true)
                        .HasColumnName("em_venda_fechada");

                    b.Property<bool>("EmVendaPerdida")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true)
                        .HasColumnName("em_venda_perdida");

                    b.Property<long>("EmpresaId")
                        .HasColumnType("bigint")
                        .HasColumnName("empresa_id");

                    b.Property<string>("Segredo")
                        .IsRequired()
                        .HasMaxLength(128)
                        .HasColumnType("character varying(128)")
                        .HasColumnName("segredo");

                    b.Property<bool>("SomenteIds")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(false)
                        .HasColumnName("somente_ids");

                    b.Property<string>("Url")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)")
                        .HasColumnName("url");

                    b.HasKey("Id");

                    b.HasIndex("EmpresaId")
                        .IsUnique()
                        .HasDatabaseName("uq_webhooks_empresa");

                    b.ToTable("webhooks_saida", (string)null);
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Auditoria", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Nexora.Core.Entidades.Usuario", "Usuario")
                        .WithMany()
                        .HasForeignKey("UsuarioId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.Navigation("Empresa");

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.CanalCaptacao", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Nexora.Core.Entidades.Conexao", "Conexao")
                        .WithMany()
                        .HasForeignKey("ConexaoId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Conexao");

                    b.Navigation("Empresa");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Conexao", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Empresa");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Contato", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Nexora.Core.Entidades.Usuario", "Responsavel")
                        .WithMany()
                        .HasForeignKey("ResponsavelId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_contatos_responsavel");

                    b.Navigation("Empresa");

                    b.Navigation("Responsavel");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.ContatoEtiqueta", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Nexora.Core.Entidades.Contato", "Contato")
                        .WithMany("Etiquetas")
                        .HasForeignKey("ContatoId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_contatos_etiquetas_contato");

                    b.HasOne("Nexora.Core.Entidades.Usuario", null)
                        .WithMany()
                        .HasForeignKey("CriadoPor", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_contatos_etiquetas_criado_por");

                    b.HasOne("Nexora.Core.Entidades.Etiqueta", "Etiqueta")
                        .WithMany()
                        .HasForeignKey("EtiquetaId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_contatos_etiquetas_etiqueta");

                    b.Navigation("Contato");

                    b.Navigation("Empresa");

                    b.Navigation("Etiqueta");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Conversa", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.CanalCaptacao", "CanalCiclo")
                        .WithMany()
                        .HasForeignKey("CanalCicloId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Nexora.Core.Entidades.Conexao", "Conexao")
                        .WithMany()
                        .HasForeignKey("ConexaoId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_conversas_conexao");

                    b.HasOne("Nexora.Core.Entidades.Contato", "Contato")
                        .WithMany()
                        .HasForeignKey("ContatoId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_conversas_contato");

                    b.HasOne("Nexora.Core.Entidades.Usuario", "UsuarioResolveu")
                        .WithMany()
                        .HasForeignKey("ResolvidoPor", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_conversas_resolvido_por");

                    b.HasOne("Nexora.Core.Entidades.Usuario", "Responsavel")
                        .WithMany()
                        .HasForeignKey("ResponsavelId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_conversas_responsavel");

                    b.Navigation("CanalCiclo");

                    b.Navigation("Conexao");

                    b.Navigation("Contato");

                    b.Navigation("Empresa");

                    b.Navigation("Responsavel");

                    b.Navigation("UsuarioResolveu");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.CredencialConversao", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Nexora.Core.Entidades.Usuario", "ConsentimentoUsuario")
                        .WithMany()
                        .HasForeignKey("ConsentimentoPor", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("fk_credenciais_consentimento");

                    b.Navigation("ConsentimentoUsuario");

                    b.Navigation("Empresa");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.EmailEnviado", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Empresa");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.EntregaWebhook", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Empresa");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.EtapaFunil", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Nexora.Core.Entidades.Pipeline", "Pipeline")
                        .WithMany()
                        .HasForeignKey("PipelineId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_etapas_pipeline");

                    b.Navigation("Empresa");

                    b.Navigation("Pipeline");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Etiqueta", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Nexora.Core.Entidades.Usuario", "UsuarioCriou")
                        .WithMany()
                        .HasForeignKey("CriadoPor", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_etiquetas_criado_por");

                    b.Navigation("Empresa");

                    b.Navigation("UsuarioCriou");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.EventoConversao", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Nexora.Core.Entidades.Contato", "Contato")
                        .WithMany()
                        .HasForeignKey("ContatoId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_conversoes_contato");

                    b.HasOne("Nexora.Core.Entidades.Negociacao", "Negociacao")
                        .WithMany()
                        .HasForeignKey("NegociacaoId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_conversoes_negociacao");

                    b.Navigation("Contato");

                    b.Navigation("Empresa");

                    b.Navigation("Negociacao");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Feriado", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("Empresa");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.FeriadoIgnorado", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Nexora.Core.Entidades.Feriado", "Feriado")
                        .WithMany()
                        .HasForeignKey("FeriadoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Empresa");

                    b.Navigation("Feriado");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.FormularioCaptura", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Empresa");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Importacao", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Nexora.Core.Entidades.Usuario", "Usuario")
                        .WithMany()
                        .HasForeignKey("UsuarioId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_importacoes_usuario");

                    b.Navigation("Empresa");

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.ImportacaoLinha", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Importacao", "Importacao")
                        .WithMany("Linhas")
                        .HasForeignKey("ImportacaoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Importacao");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Lembrete", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Nexora.Core.Entidades.Usuario", "UsuarioConcluiu")
                        .WithMany()
                        .HasForeignKey("ConcluidoPor", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_lembretes_concluido_por");

                    b.HasOne("Nexora.Core.Entidades.Contato", "Contato")
                        .WithMany()
                        .HasForeignKey("ContatoId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_lembretes_contato");

                    b.HasOne("Nexora.Core.Entidades.Conversa", "Conversa")
                        .WithMany()
                        .HasForeignKey("ConversaId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_lembretes_conversa");

                    b.HasOne("Nexora.Core.Entidades.Usuario", "UsuarioCriou")
                        .WithMany()
                        .HasForeignKey("CriadoPor", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_lembretes_criado_por");

                    b.HasOne("Nexora.Core.Entidades.Usuario", "Responsavel")
                        .WithMany()
                        .HasForeignKey("ResponsavelId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_lembretes_responsavel");

                    b.Navigation("Contato");

                    b.Navigation("Conversa");

                    b.Navigation("Empresa");

                    b.Navigation("Responsavel");

                    b.Navigation("UsuarioConcluiu");

                    b.Navigation("UsuarioCriou");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Mensagem", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Nexora.Core.Entidades.Conexao", "Conexao")
                        .WithMany()
                        .HasForeignKey("ConexaoId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_msg_conexao");

                    b.HasOne("Nexora.Core.Entidades.Contato", "Contato")
                        .WithMany()
                        .HasForeignKey("ContatoId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_msg_contato");

                    b.HasOne("Nexora.Core.Entidades.Conversa", "Conversa")
                        .WithMany()
                        .HasForeignKey("ConversaId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_msg_conversa");

                    b.HasOne("Nexora.Core.Entidades.Usuario", "UsuarioEnviou")
                        .WithMany()
                        .HasForeignKey("EnviadoPor", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_msg_enviado_por");

                    b.HasOne("Nexora.Core.Entidades.Lembrete", "Lembrete")
                        .WithMany()
                        .HasForeignKey("LembreteId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_mensagens_lembrete");

                    b.Navigation("Conexao");

                    b.Navigation("Contato");

                    b.Navigation("Conversa");

                    b.Navigation("Empresa");

                    b.Navigation("Lembrete");

                    b.Navigation("UsuarioEnviou");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Negociacao", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.CanalCaptacao", "CanalCiclo")
                        .WithMany()
                        .HasForeignKey("CanalCicloId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("fk_negociacoes_canal_ciclo");

                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Nexora.Core.Entidades.Contato", "Contato")
                        .WithMany("Negociacoes")
                        .HasForeignKey("ContatoId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_negociacoes_contato");

                    b.HasOne("Nexora.Core.Entidades.EtapaFunil", "Etapa")
                        .WithMany()
                        .HasForeignKey("EtapaId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_negociacoes_etapa");

                    b.HasOne("Nexora.Core.Entidades.Pipeline", "Pipeline")
                        .WithMany()
                        .HasForeignKey("PipelineId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_negociacoes_pipeline");

                    b.HasOne("Nexora.Core.Entidades.Usuario", "Responsavel")
                        .WithMany()
                        .HasForeignKey("ResponsavelId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_negociacoes_responsavel");

                    b.Navigation("CanalCiclo");

                    b.Navigation("Contato");

                    b.Navigation("Empresa");

                    b.Navigation("Etapa");

                    b.Navigation("Pipeline");

                    b.Navigation("Responsavel");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.NegociacaoEtiqueta", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Nexora.Core.Entidades.Usuario", null)
                        .WithMany()
                        .HasForeignKey("CriadoPor", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_negociacoes_etiquetas_criado_por");

                    b.HasOne("Nexora.Core.Entidades.Etiqueta", "Etiqueta")
                        .WithMany()
                        .HasForeignKey("EtiquetaId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_negociacoes_etiquetas_etiqueta");

                    b.HasOne("Nexora.Core.Entidades.Negociacao", "Negociacao")
                        .WithMany("Etiquetas")
                        .HasForeignKey("NegociacaoId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_negociacoes_etiquetas_negociacao");

                    b.Navigation("Empresa");

                    b.Navigation("Etiqueta");

                    b.Navigation("Negociacao");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Pipeline", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Empresa");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.RastreioLead", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("Nexora.Core.Entidades.Contato", "Contato")
                        .WithMany()
                        .HasForeignKey("ContatoId", "EmpresaId")
                        .HasPrincipalKey("Id", "EmpresaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_rastreios_contato");

                    b.Navigation("Contato");

                    b.Navigation("Empresa");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Usuario", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany("Usuarios")
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Empresa");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.WebhookSaida", b =>
                {
                    b.HasOne("Nexora.Core.Entidades.Empresa", "Empresa")
                        .WithMany()
                        .HasForeignKey("EmpresaId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Empresa");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Contato", b =>
                {
                    b.Navigation("Etiquetas");

                    b.Navigation("Negociacoes");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Empresa", b =>
                {
                    b.Navigation("Usuarios");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Importacao", b =>
                {
                    b.Navigation("Linhas");
                });

            modelBuilder.Entity("Nexora.Core.Entidades.Negociacao", b =>
                {
                    b.Navigation("Etiquetas");
                });
#pragma warning restore 612, 618
        }
    }
}

...
describe('caixa — a etiqueta da etapa', () => {
  ...
  function conversa(extra: Partial<ConversaResumo>): ConversaResumo {
    return {
      ...
      etapaId: 5, etapaNome: 'Venda', podeAbrirNegociacao: true, podeRegistrarVenda: false, contatoGanhou: true, canalDoCiclo: null,
      vendasEmAberto: 0, etiquetas: [],
      ...extra
    } as ConversaResumo;
  }
  ...
  describe('atalho de registrar venda', () => {
    it('aparece com a conversa EM ATENDIMENTO e o contato sem venda fechada', () => {
      const c = tela();
      c.sel.set(conversa({ responsavelId: 3, podeAbrirNegociacao: false, podeRegistrarVenda: true, contatoGanhou: false }));
    ...
    it('NÃO aparece em conversa sem dono', () => {
      const c = tela();
      c.sel.set(conversa({ responsavelId: null, podeAbrirNegociacao: false, podeRegistrarVenda: true, contatoGanhou: false }));
    ...
    it('NÃO aparece para quem já tem venda fechada — ali o caminho é abrir nova negociação', () => {
      const c = tela();
      c.sel.set(conversa({ responsavelId: 3, podeAbrirNegociacao: true, podeRegistrarVenda: false, contatoGanhou: true }));
      expect(c.podeRegistrarVenda()).toBeFalse();
    });
  ...
  it('quem nunca comprou mostra a etapa normalmente', () => {
    const c = conversa({ podeAbrirNegociacao: false, podeRegistrarVenda: true, contatoGanhou: false, etapaNome: 'Proposta', vendasEmAberto: 0 });
